...
library(data.table) 
library(Seurat)
setwd("/home/chengyq/excercise/WGCNA/covid_result/")
### 启用WGCNA多核计算
enableWGCNAThreads(nThreads = 0.75*parallel::detectCores()) 
getwd()
################################# 0.输入数据准备 ################################
seurat.obj <- readRDS("/home/zhangtl/Heart_covid/output/cell_type/CM/seurat/cm_new0405_last.rds")
# 查看当前分组
...
expr2 <- as.data.frame(AverageExpression(seurat.obj, group.by = c("group","donor"), assays = "RNA")[["RNA"]])
name2 <- as.data.frame(colnames(expr2))
name2 <- str_split_fixed(name2$`colnames(expr2)`,"_",2)
name2 <- as.data.frame(name2)
table(name2$V1)
# CHD    DCM    HCM     MI Normal 
# 16     20     30     25    132
# HF HM NF NM 
# 10 20 19 11 
 #把命名赋值给expr
...
if(T){
  ...
  # 1773 1636  795  623  574  408  295  209  194  130   86   68   59   51   50   49 
...
#条形图
if(T){
  y = as.data.frame(design[ ,"COVID-19"]);
  GS1=as.numeric(cor(y,datExpr,use="p"))
  GeneSignificance=abs(GS1)
  # Next module significance is defined as average gene significance.
  ModuleSignificance=tapply(GeneSignificance,moduleColors , mean , na.rm=T)
  pdf("step4-Module-single_trait-relationship_barplot_HF.pdf",width = 8,height = 5)
...
if(T){
  ...
  if(T){
    ...
    # design
    HF = as.data.frame(design[,1])
    names(HF) = "COVID-19"
    # Add the weight to existing module eigengenes
    MET = orderMEs(cbind(MEs, HF))
  }
  pdf("step5_module_cor_Eigengene-dendrogram_COVID-19.pdf",width = 8,height = 10)
...
# 209          795          623           50          408           68         1773          130           49 
# pink       purple          red       salmon          tan    turquoise       yellow 
# 194           86          295           51           59         1636          574 
