...
setwd("/home/chengyq/Pericyte/results/WGCNA/")
### 启用WGCNA多核计算
enableWGCNAThreads(nThreads = 0.75*parallel::detectCores()) 
getwd()
################################# 0.输入数据准备 ################################
seurat.obj <- readRDS("/home/chengyq/Pericyte/data/merge/sub_harmony_10_res0.1.rds")
# 查看当前分组
Idents(seurat.obj) <- "disease"
table(Idents(seurat.obj))

#seurat.obj <- subset(seurat.obj,subset = sub_cell_type %in% "CM1")
seurat.obj <- NormalizeData(seurat.obj)
seurat.obj@meta.data
#按照样本取平均，获得表达矩阵
expr <- AverageExpression(seurat.obj, group.by = "donor", assays = "RNA")[["RNA"]]
expr <- as.data.frame(expr)
dim(expr)
# 更改矩阵行名为样本＋疾病，为表型数据做准备
expr2 <- as.data.frame(AverageExpression(seurat.obj, group.by = c("disease","donor"), assays = "RNA")[["RNA"]])
name2 <- as.data.frame(colnames(expr2))
name2 <- str_split_fixed(name2$`colnames(expr2)`,"_",2)
name2 <- as.data.frame(name2)
table(name2$V1)
# CS    DCM    HCM    ICM Normal 
# 4     11     15      7     14 
# HF HM NF NM 
# 10 20 19 11 
#把命名赋值给expr
...
# CS574#CS   0.01783586  0.6238990 0.101673828    0.05433560
# CS676#CS   0.05093512  0.5831457 0.164022701    0.12695694
# CS707#CS   0.00000000  0.4773649 0.091044050    0.02471867
# CS761#CS   0.02457802  0.6716092 0.009064375    0.22485185
#                 MIR1302-2HG FAM138A OR4F5 AL627309.1
# LV_1422_1_hcm#HF           0       0     0 0.01653924
# LV_1422_2_hcm#HF           0       0     0 0.01445973
# LV_1425_1_hcm#HF           0       0     0 0.01740739
# LV_1425_2_hcm#HF           0       0     0 0.01234250
### 筛选数据前7000的基因
expr <- t(expr)
...
dim(keep_data)
keep_data[1:4,1:4]
# MSC-AS1     FRYL     SMC5    USP47
# CS574#CS  9.395478 4.551248 3.560974 4.267047
# CS676#CS 10.808499 4.813827 2.250809 5.217991
# CS707#CS  9.497317 3.208406 2.580104 4.324700
# CS761#CS  6.973807 7.052552 3.094201 4.638708
#                    MALAT1     RYR2      TTN    FGF12
# LV_1422_1_hcm#HF 838.7505 241.1707 238.0153 57.15413
# LV_1422_2_hcm#HF 849.9423 243.5381 241.6382 56.67540
# LV_1425_1_hcm#HF 861.9459 183.2709 276.5260 32.40090
# LV_1425_2_hcm#HF 891.2555 172.8990 263.1397 32.24253
...
#                  group groupNo
# LV_1422_1_hcm#HF    HF       1
# LV_1422_2_hcm#HF    HF       1
# LV_1425_1_hcm#HF    HF       1
# LV_1425_2_hcm#HF    HF       1
# LV_1447_1_hcm#HF    HF       1
# LV_1447_2_hcm#HF    HF       1
table(datTraits$group)
...
if(T){
  ...
  # 336 1991 1299  855  436  408  355  317  282  172  139  104  101   95   72   38 
...
datTraits$group
#datTraits$group <- ifelse(grepl("Normal#$", rownames(datTraits)), "Normal", "COVID-19")#数据框 datTraits 添加一个名为 group 的新列，根据条件判断将每行数据分为两类：如果行名（rownames）以 "Normal#" 结尾，则在 group 列中标记为 "Normal"；否则标记为 "COVID-19"。
## 模块与表型的相关性热图
if(T){
...
if(T){
  ...
  pdf("step4-Module-single_trait-relationship_barplot_HCM.pdf",width = 8,height = 5)
...
if(T){
  ...
  draw_ggboxplot <- function(data,Module="Module",group="group"){
    ggboxplot(data,x=group, y=Module,
              ...
              xlab = group,
              fill = group,
              palette = c("#E64B35CC","#4DBBD5CC", "#00A087CC","#F39B7FCC","#D9D9D9"),
              #palette = "jco",
              #add="jitter",
  ...
  do.call(grid.arrange,c(p,ncol=4)) #排布为每行2个
  dev.off()
}
...
if(T){
  ...
  pdf("step4_gene-Module-trait-significance_DCM.pdf",width=7, height=1.5*ncol(MEs))
...
if(T){
  ...
  if(T){
    ...
    # design
    DCM = as.data.frame(design[,1])
    names(DCM) = "DCM"
    # Add the weight to existing module eigengenes
    MET = orderMEs(cbind(MEs, DCM))
  }
  pdf("step5_module_cor_Eigengene-dendrogram_DCM.pdf",width = 8,height = 10)
...
genetype = "SYMBOL"    # "SYMBOL"   "ENSEMBL"  ALIAS
table(moduleColors)
# moduleColors
# black         blue        brown         cyan        green  greenyellow         grey      magenta 
# 317         1299          855           72          408          104          336          172 
# midnightblue         pink       purple          red       salmon          tan    turquoise       yellow 
# 38          282          139          355           95          101         1991          436 



choose_module <- c("black","blue","brown","cyan","green",
                   "purple","red") 
if(T){
  library(clusterProfiler)
...
#######WGCNA种提取模块gene进行打分-----
modulegene <- read.csv("~/excercise/WGCNA/covid_result/step6_gene_moduleColors.csv")
modulegene <- subset(modulegene,subset = module %in% "green") %>% pull(gene)
library(ggplot2)
library(ggpubr)
####模块基因进行功能富集-----
library(DOSE)
...
library(ggplot2)
# Cluster2
# term <- readRDS("~/ALLEN_SEX/data/monocle2/term/term2.rds")
go <- c(
  # "muscle contraction",  #ACE2
  "cardiac muscle contraction", #ACE2
  # "positive regulation of cardiac muscle contraction", #ACE2
  "heart contraction", #ACE2
  # "positive regulation of MAP kinase activity", #MAP2
  "positive regulation of MAPK cascade", #MAP2
  "insulin-like growth factor receptor signaling pathway", #MAP2
  "positive regulation of MAPK cascade", #C5AR1
  # "myeloid leukocyte activation", #C5AR1
  "glycerolipid biosynthetic process", #INPP5F
  # "phospholipid metabolic process", #INPP5F
  # "muscle hypertrophy",   #INPP5F
  "cardiac muscle hypertrophy", #INPP5F
  # "protein kinase B signaling", #INPP5F
  "calcium ion homeostasis", #BCL2
  # "calcium ion transport", #BCL2
  # "actin filament bundle assembly", #TJP1
  # "actin filament organization", #ACTN1
  # "cell-substrate adhesion", #ACTN1
  "cell-matrix adhesion", #ACTN1
...
p <- ggplot(data = term,
  ...
  geom_bar(stat="identity", alpha=1, fill= "#a3d393",width = 0.8) +
  geom_text(aes(x=labelx, y=labely, label = term$Description),size=3.5,hjust =0)+
  theme_classic()+
